...
class Commodity(OSeMOSYSBase):
    ...
    def to_otoole_csv(self, comparison_directory) -> "cls":
        ...
        if not os.path.isfile(os.path.join(comparison_directory, 'SpecifiedAnnualDemand.csv')):
            pd.DataFrame(columns=["REGION","FUEL","YEAR","VALUE"]).to_csv(os.path.join(comparison_directory, 'SpecifiedAnnualDemand.csv'), index=False)
        if self.demand_annual is not None:
            # Read existing dataframe
            df_demand_annual = pd.read_csv(os.path.join(comparison_directory, 'SpecifiedAnnualDemand.csv'))
            ...
            # Write dataframe
            df_demand_annual.to_csv(os.path.join(comparison_directory, 'SpecifiedAnnualDemand.csv'), index=False)

    
        # demand_profile
        # Create output dataframe if not already existing
        if not os.path.isfile(os.path.join(comparison_directory, 'SpecifiedDemandProfile.csv')):
            pd.DataFrame(columns=["REGION","FUEL","TIMESLICE","YEAR","VALUE"]).to_csv(os.path.join(comparison_directory, 'SpecifiedDemandProfile.csv'), index=False)
        if self.demand_profile is not None:
            # Read existing dataframe
            df_demand_profile = pd.read_csv(os.path.join(comparison_directory, 'SpecifiedDemandProfile.csv'))
            # Convert JSON data object to df
            ...
            df_demand_profile_to_add = df_demand_profile_to_add[["REGION","FUEL","TIMESLICE","YEAR","VALUE"]]
            # Add to dataframe
            df_demand_profile = pd.concat([df_demand_profile, df_demand_profile_to_add])
            # Write dataframe
            df_demand_profile.to_csv(os.path.join(comparison_directory, 'SpecifiedDemandProfile.csv'), index=False)

        # accumulated_demand
        # Create output dataframe if not already existing
        if not os.path.isfile(os.path.join(comparison_directory, 'AccumulatedAnnualDemand.csv')):
            pd.DataFrame(columns=["REGION","FUEL","YEAR","VALUE"]).to_csv(os.path.join(comparison_directory, 'AccumulatedAnnualDemand.csv'), index=False)
        if self.accumulated_demand is not None:
            # Read existing dataframe
            df_accumulated_demand = pd.read_csv(os.path.join(comparison_directory, 'AccumulatedAnnualDemand.csv'))
            # Convert JSON data object to df
            ...
            df_accumulated_demand_to_add = df_accumulated_demand_to_add[["REGION","FUEL","TIMESLICE","YEAR","VALUE"]]
            # Add to dataframe
            df_accumulated_demand = pd.concat([df_accumulated_demand, df_accumulated_demand_to_add])
            # Write dataframe
            df_accumulated_demand.to_csv(os.path.join(comparison_directory, 'AccumulatedAnnualDemand.csv'), index=False)
